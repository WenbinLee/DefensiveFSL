...
class CarliniWagnerL2:
    ...
    def _step(self, model: nn.Module, optimizer: optim.Optimizer, inputs: torch.Tensor, inputs2: torch.Tensor, tinputs: torch.Tensor,
        ...
        logits = model(adv_input, inputs2)
    ...
    def attack(self, model: nn.Module, inputs: torch.Tensor, inputs2: torch.Tensor, labels: torch.Tensor,
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            for iteration in range(self.max_iterations):
                # perform the attack
                adv, logits, l2, logit_dists, loss = self._step(model, optimizer, inputs, inputs2, tinputs, modifier,
    ...
    def _quantize(self, model: nn.Module, inputs: torch.Tensor, inputs2: torch.Tensor, adv: torch.Tensor, labels: torch.Tensor,
                  targeted: bool = False) -> torch.Tensor:
        """
        ...
        logits = model(inputs + delta, inputs2)
