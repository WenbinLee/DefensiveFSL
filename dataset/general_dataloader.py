...
def data_split(data_dir, class_img_dict, class_list, class_to_idx, mode):
	...
	for index, class_item in enumerate(class_list):
		...
		train_part = random.sample(temp_list, int(0.9*len(temp_list)))
		val_part = [val for val in temp_list if val not in train_part]
	...
	for mode in modes:
		...
		if opt.train_aug and opt.imageSize == 224 and mode == 'train':
			...
			transform = transforms.Compose([
				...
				transforms.RandomCrop(opt.imageSize),
				# transforms.RandomCrop(opt.imageSize, padding=8),
				transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
				transforms.RandomHorizontalFlip(),
				transforms.ToTensor()
				# transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
	...
	for mode in modes:
		...
		if opt.train_aug and opt.imageSize == 224 and mode == 'train':
			...
			transform = transforms.Compose([
				...
				# transforms.RandomCrop(opt.imageSize, padding=8),
				transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
				# transforms.RandomApply([transforms.GaussianBlur(kernel_size=opt.imageSize//20*2+1, sigma=(0.1, 2.0))], p=0.5),
				transforms.RandomHorizontalFlip(),
				transforms.ToTensor()
				# transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
				# transforms.Normalize(mean=mean, std=std)
