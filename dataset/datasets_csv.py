	...
	def __init__(self, data_dir="", mode="train", image_size=84, data_name="miniImageNet",
		...
		if mode == "train":
			...
			while e < episode_num:
				...
				label_num = -1 
				class_to_idx = {i:temp_list[i] for i in range(len(temp_list))}
	...
	def __getitem__(self, index):
		...
		for i in range(len(episode_files)):
			...
			for j in range(len(query_dir)):
				...
				# temp_img = np.uint8((np.asarray(temp_img)-128.))
		...
		classes = episode_files[0]['classes']
		class_to_idx = episode_files[0]['class_to_idx']

		# Shuffle the query images 
		# rand_num = torch.rand(1)
		# random.Random(rand_num).shuffle(query_images)
		# random.Random(rand_num).shuffle(query_targets)           
		return (query_images, query_targets, support_images, support_targets, classes, class_to_idx)